...
router.get('/', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const enhancedIdeas = ideas.map(idea => ({
      ...idea.ideas,
      creator: {
        id: idea.creator?.id || null,
        name: idea.creator?.name || '',
        email: idea.creator?.email || '',
        photoUrl: idea.creator?.photoUrl || null
      },
      responsible: idea.responsible_user ? {
        id: idea.responsible_user.id,
        name: idea.responsible_user.name,
        email: idea.responsible_user.email,
        photoUrl: idea.responsible_user.photoUrl
      } : null,
      group: idea.groups ? {
        id: idea.groups.id,
