...
router.get('/', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    let query = db.select().from(schema.ideas)
      .leftJoin(schema.users.as('creator_users'), eq(schema.ideas.creatorId, schema.users.as('creator_users').id))
      .leftJoin(schema.users.as('responsible_users'), eq(schema.ideas.responsibleId, schema.users.as('responsible_users').id))
...
router.get('/:id', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const comments = await db.select()
      .from(schema.ideaComments)
      .leftJoin(schema.users.as('comment_users'), eq(schema.ideaComments.userId, schema.users.as('comment_users').id))
    ...
    const volunteers = await db.select()
      .from(schema.ideaVolunteers)
      .leftJoin(schema.users.as('volunteer_users'), eq(schema.ideaVolunteers.userId, schema.users.as('volunteer_users').id))
...
// Resto do código permanece similar, apenas atualizando os aliases quando necessário
