...
- **Gamification Category-Based Challenge Visibility**: Implemented comprehensive category-based challenge visibility system for gamification. Added `targetUserCategories` array field to gamification challenges schema allowing administrators to specify which user categories should see specific challenges. Enhanced admin interface with visual checkbox selection for target user categories during challenge creation and editing. Implemented filtering logic in both challenge list and individual challenge endpoints to ensure users only see challenges for their assigned categories or unrestricted challenges (those without specific target categories). Fixed runtime errors in ChallengeEvaluationForm by adding safety checks for undefined data
- **Admin Table UX Enhancement with Vertical Scroll**: Implemented fixed-header scrollable tables in all admin management pages (Users, Categories, Groups) to solve sidebar visibility issues during long list navigation. Tables now feature separated header and body sections with the header remaining fixed while content scrolls vertically within a 500px maximum height container. This prevents the sidebar from disappearing when scrolling through extensive user lists, maintaining consistent navigation throughout the admin interface. Enhanced visual design with proper borders and background colors for better content organization and readability.r file configuration. **Important**: Administrators and superadministrators always see all challenges regardless of category restrictions, ensuring proper admin management capabilities. System works as designed: challenges without specified categories are visible to all users, while challenges with specific categories are only visible to users assigned to those categories or admins.
- **User Profile Join Date Display**: Enhanced user profile pages to display "Data de Ingresso" (Join Date) field. Updated both UserProfileModal (for admin viewing other users) and Profile page (for users viewing their own profile) to show formatted join date based on user's createdAt timestamp. Changed label from "Cadastrado em" to more appropriate "Data de Ingresso" and added proper visual separation with dividers for better information hierarchy. Users can now clearly see when they joined the platform in a user-friendly format using Brazilian Portuguese date formatting.
- **Trail Category-Based Visibility Control**: Implemented comprehensive trail visibility management system where trails can be restricted to specific user categories. Added targetUserCategories field to trails schema allowing administrators to specify which user categories can view each trail. Frontend includes multi-select category picker in admin forms. Backend implements intelligent filtering logic: regular users see only trails without category restrictions or trails matching their assigned categories, while administrators and superadministrators maintain full access to all trails regardless of category restrictions. System includes getUserCategoryAssignments method in storage layer for efficient category-based permission checking.
...
O portal agora possui um sistema completo de backup automático implementado:

- **scripts/backup.js**: Backup completo (banco + arquivos + configurações)
- **scripts/restore.js**: Restauração interativa seletiva  
- **scripts/backup-scheduler.js**: Agendamento automático com cron
- **docs/backup-system.md**: Documentação completa

Recursos: retenção de 7 dias, confirmações de segurança, logs detalhados, múltiplas opções de agendamento.
Execução: `node scripts/backup.js` para backup manual ou `node scripts/backup-scheduler.js daily` para automático.
