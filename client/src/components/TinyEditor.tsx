...
const TinyEditor: React.FC<TinyEditorProps> = ({ value, onChange, className }) => {
  ...
  const initEditor = () => {
    ...
    window.tinymce.init({
      ...
      setup: (editor) => {
        editor.on('init', () => {
          console.log("TinyMCE inicializado com conteúdo:", initialValueRef.current);
  ...
  useEffect(() => {
    // Salvar o valor inicial em uma ref para uso após carregamento do script
  ...
  // Atualizar o conteúdo quando o valor muda externamente
  useEffect(() => {
    if (editorInitialized && window.tinymce && window.tinymce.get(editorId.current)) {
